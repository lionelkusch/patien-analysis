...
def pipeline_spectral(path_saving, patients_data, selected_subjects,
             ...
             kmeans_nb_cluster=5, kmeans_seed=123, plot=False, plot_save=True,
    ...
    # all data
    if update_Phate or not os.path.exists(path_saving + "/spherical.npy"):
        spectral_operator = SpectralClustering(n_clusters=kmeans_nb_cluster, n_jobs=PHATE_n_jobs, random_state=kmeans_seed,
                                               affinity=PHATE_knn_dist, gamma=PHATE_gamma, n_neighbors=PHATE_knn)
        Y_spherical = spectral_operator.fit(np.concatenate(avalanches_bin)).labels_
        np.save(path_saving + "/spherical.npy", Y_spherical)
    else:
        Y_spherical = np.load(path_saving + "/spherical.npy")
...
if __name__ == '__main__':
    ...
    path_saving = path_data + "/../paper/result/spectral_rbf/"

    # pipeline_spectral(path_saving, data, selected_subjects,
    #              avalanches_threshold=3, avalanches_direction=0, avalanches_binsize=1,
    #              PHATE_n_pca=5, PHATE_knn=5, PHATE_decay=1.0, PHATE_knn_dist='rbf',
    #              PHATE_gamma=-1.0, PHATE_mds_dist='cosine', PHATE_n_components=3, PHATE_n_jobs=8,
    #              kmeans_nb_cluster=7, kmeans_seed=123,
    #              update_avalanches=False, update_Phate=True, update_transition=True,
    #              plot=True, plot_save=True)

    for i in range(3, 15):
        path_saving = path_data + "/../paper/result/spectral_cosine/spectral_cosine"+str(i)+"/"
        ...
        pipeline_spectral(path_saving, data, selected_subjects,
                   ...
                   PHATE_n_pca=5, PHATE_knn=5, PHATE_decay=1.0, PHATE_knn_dist='cosine',
                   PHATE_gamma=-1.0, PHATE_mds_dist='cosine', PHATE_n_components=3, PHATE_n_jobs=8,
                   kmeans_nb_cluster=i, kmeans_seed=123,
                   update_avalanches=False, update_Phate=True, update_transition=True,
                   plot=True, plot_save=True)
