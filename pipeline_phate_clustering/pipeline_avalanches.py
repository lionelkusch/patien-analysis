...
from pipeline_phate_clustering.functions_helper.plot import plot_figure_2D, plot_figure_2D_patient, \
    plot_figure_2D_patient_unique, plot_figure_2D_patient_unique_time, plot_figure_2D_3D, plot_figure_3D_patient


def pipeline_avalanches(path_saving, patients_data, selected_subjects,
    ...
    """
    Pipeline for extracting unsupervised clustering from resting state MEG data based on avalanches
    and not avalanches patterns
    ...
    if update_avalanches or not os.path.exists(path_saving + '/avalanches_only.npy') or \
        ...
        avalanches = np.array(data)
        np.save(path_saving + '/avalanches_only.npy', avalanches)
    else:
        if avalanches_path is not None:
            avalanches = np.load(avalanches_path, allow_pickle=True)
        else:
            avalanches = np.load(path_saving + '/avalanches_only.npy', allow_pickle=True)

    if plot:
        plt.figure()
        plt.bar(np.arange(0, 90, 1), np.sum(np.concatenate(avalanches), axis=0))
    ...
    if update_Phate or not os.path.exists(path_saving + "/Phate.npy"):
        ...
        Y_phate = phate_operator.fit_transform(np.concatenate(avalanches))
    ...
    if plot:
        ...
        if plot_save:
            ...
            plt.close('all')
        plot_figure_2D_patient(Y_phate, '', avalanches)
        if plot_save:
            plt.savefig(path_saving + '/figure/cluster_for_patient.png');
            plt.close('all')
        plot_figure_3D_patient(Y_phate, '', avalanches)
        if plot_save:
            plt.savefig(path_saving + '/figure/cluster_patient_3D.png', dpi=600);
            plt.close('all')
        plot_figure_2D_patient_unique(Y_phate, '', avalanches)
        if plot_save:
            plt.savefig(path_saving + '/figure/cluster_for_patient_unique.png');
            plt.close('all')
        plot_figure_2D_patient_unique_time(Y_phate, '', avalanches)
        if plot_save:
    ...
    if update_transition or not os.path.exists(path_saving + "/transition.npy"):
        ...
        for j in range(kmeans_nb_cluster):
            histograms_region.append(np.sum(np.concatenate(avalanches)[np.where(cluster == j)], axis=0))
    ...
    if plot:
        ...
        plt.imshow(histograms_region / histograms_region.max(axis=0).reshape(1, len(avalanches[0][0])))
        if plot_save:
    ...
    if save_for_matlab:
        ...
        io.savemat(path_saving + '/data.mat',
                   {'source_reconstruction_MEG': patients_data,
                    'avalanches_binarize': avalanches,
